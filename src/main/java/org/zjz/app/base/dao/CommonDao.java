...
public class CommonDao {
	 ...
	 */
	private <T> Criteria createCriteria(Class<T> entityClass, final Criterion... criterions) {
	 ...
	 * 按id删除对象.
	 */
	public <T, PK extends Serializable> void delete(Class<T> entityClass, final PK id) {
	 ...
	 * 按属性查找对象列表, 匹配方式为相等.
	 */
	public <T> List<T> findByPorperty(Class<T> entityClass, final String propertyName, final Object value) {
		Assert.hasText(propertyName, "propertyName不能为空");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T> Page<T> findPageByHql(final Page<T> page, String hql, final Map<String, ?> values) {
		Assert.notNull(page, "page不能为空");
		// 排序
		String order = "";
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T> Page<T> findPageBySql(final Page<T> page, String sql, final Map<String, ?> values) {
		Assert.notNull(page, "page不能为空");
		// 排序
		String order = "";
		if (page.isOrderBySetted()) {
	 ...
	 * @return
	 */
	public <T, PK extends Serializable> T findOne(Class<T> entityClass, final PK id) {
		Assert.notNull(id, "id不能为空");
	 ...
	 * 
	 */
	public <T> Page<T> findAll(Class<T> entityClass, final Page<T> page) {
	 ...
	 */
	public <T> boolean isPropertyUnique(Class<T> entityClass, final String propertyName, final Object newValue,
			final Object oldValue) {
		if ((newValue == null) || newValue.equals(oldValue)) {
			return true;
		}
		Object object = findUniqueByProperty(entityClass, propertyName, newValue);
		return (object == null);
	}
	 ...
	 */
	private <T> Criteria setPageParameterToCriteria(final Criteria c, final Page<T> page) {
	 ...
	 * 设置分页参数到Query对象,辅助函数.
	 */
	private <T> Query setPageParameterToQuery(final Query q, final Page<T> page) {
		Assert.isTrue(page.getPageSize() > 0, "Page Size must larger than zero");
		// hibernate的firstResult的序号从0开始
